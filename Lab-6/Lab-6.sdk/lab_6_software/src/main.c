...
void DemoRun() {
   ...
   while (1) {
	  time = GetTime(&myDeviceRTCC, RTCC_TARGET_RTCC);
	  timeString[0]= '0'+bcd2int(time.hour)/10;
	  timeString[1] = '0'+bcd2int(time.hour)%10;
	  timeString[2] = ':';
	  timeString[3] = '0'+bcd2int(time.minute)/10;
	  timeString[4] = '0'+bcd2int(time.minute)%10;
	  timeString[5] = ':';
	  timeString[6] = '0'+bcd2int(time.second)/10;
	  timeString[7] = '0'+bcd2int(time.second)%10;
	  timeString[9] = 'M';
	  if (time.ampm == 1){
			  	  timeString[8] = 'A';
	  }
      ...
      OLED_PutString(&myDeviceOLED, timeString);
